...
namespace WhatINoted.Tests
{
    class GoogleBooksConnectionTests
    {
        ...
        private class GoogleBooksTestSearchVolumes : Test
        {
            public bool Run(StreamWriter sw)
            {
                return SearchVolumesTitle(sw) && SearchVolumesAuthor(sw) && SearchVolumesPublisher(sw)
                    && SearchVolumesIsbn(sw) && SearchVolumesTitleAuthor(sw)
                    && SearchVolumesTitlePublisher(sw) && SearchVolumesTitleIsbn(sw)
                    && SearchVolumesAuthorPublisher(sw) && SearchVolumesAuthorIsbn(sw)
                    && SearchVolumesPublisherIsbn(sw) && SearchVolumesTitleAuthorPublisher(sw)
                    && SearchVolumesTitleAuthorIsbn(sw) && SearchVolumesTitlePublisherIsbn(sw)
                    && SearchVolumesAuthorPublisherIsbn(sw) && SearchVolumesAll(sw)
                    && SearchVolumesNullParams(sw) && SearchVolumesEmptyParams(sw);
            }
